...
// 	cout << "CbParam: " << tmpParam->iExg << " " << tmpParam->sExg << endl; // 仅表示自定义参数示例
	...
	cout<<"the mainUUID"<<pmainUUID<<endl;
	//switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, " OnResultDataRecved %s %s\n", sn.c_str(), json_result.c_str());
	switch_event_t *event = NULL;
	if (switch_event_create(&event, SWITCH_EVENT_CUSTOM) == SWITCH_STATUS_SUCCESS) {

		event->subclass_name = strdup("asr");
		switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Event-Subclass", event->subclass_name);
		switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "ASR-Response", cbEvent->getResult());
    ...
    case SWITCH_ABC_TYPE_INIT:
        {
			...
			pvt->callback->setOnTranscriptionStarted(onTranscriptionStarted,(void*)((const char*)pvt->mainUUID)); // 设置识别启动回调函数
			pvt->callback->setOnTranscriptionResultChanged(onTranscriptionResultChanged, (void*)((const char*)pvt->mainUUID)); // 设置识别结果变化回调函数
			pvt->callback->setOnTranscriptionCompleted(onTranscriptionCompleted, (void*)((const char*)pvt->mainUUID)); // 设置语音转写结束回调函数
			pvt->callback->setOnSentenceBegin(onSentenceBegin,(void*)((const char*)pvt->mainUUID)); // 设置一句话开始回调函数
			pvt->callback->setOnSentenceEnd(onSentenceEnd,(void*)((const char*)pvt->mainUUID)); // 设置一句话结束回调函数
			pvt->callback->setOnTaskFailed(onTaskFailed, (void*)((const char*)pvt->mainUUID)); // 设置异常识别回调函数
			pvt->callback->setOnChannelClosed(onChannelClosed,(void*)((const char*)pvt->mainUUID)); // 设置识别通道关闭回调函数
