...
void* pthreadFunc(void* arg) {
	...
	*/
	...
	request->setUrl(tst->url.c_str()); // 设置服务端url, 必填参数
...
* 识别单个音频数据
*/
int speechRecognizerFile(const char* url) {
...
int speechRecognizerMultFile(const char* url) {
...
int main(int arc, char* argv[]) {
	if (arc < 2) {
		cout << "params is not valid. Usage: ./demo <url>" << endl;
		return -1;
	}
	...
	// 识别单个音频数据
	speechRecognizerFile(url.c_str());

	// 识别多个音频数据
	//speechRecognizerMultFile(url.c_str());
